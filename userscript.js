    ...
    setTimeout(function () {
        ...
        WebSocket.prototype.send = function (data) {
            ...
            this.send = function (data) {
                ...
                if (this.url.includes('localhost')) return; // cant get ip from localhost?
                // double check above line ****

                if (data instanceof ArrayBuffer) data = new DataView(data); // fixes "first parse cannot be arraybuffer"
                else if (data instanceof DataView) data = data; // idk what this does but ig means we ave the right type of data
                ...
                if (data.byteLength === 5 || data.byteLength < 4) {
                    if (data.getUint8(0, true) === 16) {
